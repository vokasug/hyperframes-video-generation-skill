...
17. **Правило подсветки текста**: в кадре — ОДИН механизм акцента. Если предложения
    определения подсвечиваются фоном под диктора, то `<b>` внутри текста НЕ крась акцентом
    (оставь тёмным) — цветной полужирный зритель считывает как «второе подсвеченное
    предложение». Подсветка — ровно одно предложение в момент времени: на метке предложения
    гасить ВСЕ `.a-sent`, включать только текущее. Маппинг предложение-экрана ↔ предложение
    озвучки — **1:1 по индексу** (не пропорциональный!); абзацы экрана объединяй так, чтобы
    их число совпадало с числом фраз VO.
18. **Анимации — только два паттерна, никаких `stagger+repeat+yoyo`** (случайное мелькание
    выглядит дёшево и раздражает):
    а) **VO-sync**: подсвечивается только элемент, о котором диктор говорит сейчас
       (иконка/строка/карточка), остальное статично;
    б) **плавный последовательный цикл**: sweep слева направо по элементам списка
       (дискретные шаги по порядку, цикл), пакеты СТРОГО по прямым линиям
       (`attr:{cx,cy}` от точки к точке), медленные float/blink, бесшовные marquee
       (дублированный контент + линейный `fromTo y:0→-H` с repeat).
    Пульсирующая «подсветка-бегущий-огонёк» идёт по элементам ПО ПОРЯДКУ и повторяется,
    никогда — вразнобой.
19. Синхронизация иконок/строк с VO: VO почти всегда имеет вводную фразу → маппинг
    **с выравниванием по хвосту**: элемент i ↔ предложение (min(1, len(sents)−n) + i).
    Проверяй численно (getComputedStyle в браузере на конкретных таймкодах), а не глазами
    по миниатюрам — базовый оттенок ~10% легко спутать с подсветкой 28%.
20. Геометрия, которая вечно «уезжает» (проверяй zoom-снапшотом каждый раз):
    - ручка слайдера: `top = высота_строки_меток + (высота_линии − диаметр_ручки)/2`
      (строка меток НЕ ноль — типовая ошибка на ~35px);
    - стрелка под иконкой: left = центр иконки − половина ширины глифа (~7px);
    - SVG обрезается снизу: последний узел y + r > height — увеличь height с запасом;
    - точки на линиях: линии делай ПРЯМЫМИ и твинь `attr cx/cy` ровно по сегментам —
      кривые Безье + линейный cx/cy = точка «летит мимо».
