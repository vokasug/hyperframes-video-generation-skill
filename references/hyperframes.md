...
14. **Никакого SMIL** (`<animate>`, `<animateMotion>`): не seek-безопасен, ломает детерминизм
    рендера и может подвесить страницу. Движение по траектории — только GSAP-твинами
    `attr:{cx,cy}` (движение по ломаной = цепочка твинов по сегментам с repeat от ОСТАТКА).
15. **«Тяжёлые оверлеи» (CSS `filter:blur`, `radial-gradient`, `clip-path`) — суммарно ≤ ~35
    на всю композицию**, включая скрытые opacity:0 (считаются все, кроме display:none).
    При ~40+ элементы захвата рендерятся чёрными первую половину видео. Размытые цветные
    пятна-орбы и градиентные споты давай ТОЛЬКО ключевым сценам (титул, интро глав, финал),
    не каждой сцене.
16. Навигационный таймаут на больших проектах (60+ сцен, десятки медиафайлов): check/snapshot/
    render периодически падают с `Navigation timeout of 10000 ms exceeded` — это флаки
    загрузки, лечится `PRODUCER_PAGE_NAVIGATION_TIMEOUT_MS=120000` перед командой.
...
- `Navigation timeout` на большом проекте → правило 16 (env-переменная).
- Чёрные кадры в первой половине рендера → слишком много тяжёлых оверлеев (правило 15).
- `[StaticGuard] Invalid HyperFrame contract: <audio> uses data-end... / Font families used without
  @font-face` — НЕ фатал: это warning компилятора, который сам генерирует data-end и инлайнит
  fonts.css уже после проверки. Игнорируй, если check в остальном зелёный.
- `content_overlap` бейджа с заголовком на px → сдвинь бейдж выше/термин ниже: бокс заголовка
  больше глифов (учитывай line-height).
